...
const NoteCard = ({note, onOpen, onToggleFavourite, activeMenu, setActiveMenu}) => {
    return(
        <div className='relative flex flex-col rounded-xl border border-black/5 bg-white p-4 shadow-sm transition-shadow hover:shadow-md'>
                <div className='mb-2 flex items-start justify-between gap-2'>
                    ...
                    <button type='button' onClick={(e) => {e.stopPropagation(); onToggleFavourite?.(note);}} className='text-ink/30 hover:text-clay' aria-label='Toggle favourite'>
                            <Star size={16} fill={note.isFavourite ? '#b08968' : 'none'} className= {note.isFavourite? 'text-clay' : ''}/>
                        </button>
                </div>
                <button type='button' onClick={()=> onOpen?.(note)} className='flex-1 text-left'><p className='mb-4 line-clamp-2 flex-1 text-xs text-ink/60'>{stripMarkdown(note.content) || 'No additional content yet.'}</p></button>
                ...
                <div className='flex items-center justify-between'>
                    ...
                    {activeMenu === note.id && (
                        <div className='absolute right-0 top-7 z-20 w-40 rounded-lg border border-black/10 bg-white shadow-lg'>
                            <button className="block w-full px-4 py-2 text-left text-sm hover:bg-sand/40" onClick={()=> {alert('archive clicked'); setActiveMenu(null);}}>Archive</button>
                            <button className='block w-full px-4 py-2 text-left text-sm hover:bg-sand/40' onClick={()=>{alert('delete clicked'); setActiveMenu(null)}}>Delete</button>
                        </div>
                    )}
                </div>
                </div>
