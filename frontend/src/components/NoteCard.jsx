...
const NoteCard = ({note, onOpen, onToggleFavourite, activeMenu, setActiveMenu}) => {
    return(
        <div className='relative flex flex-col rounded-xl border border-black/5 bg-white p-4 shadow-sm transition-shadow hover:shadow-md'>
                ...
                <button type='button' onClick={()=> onOpen?.(note)} className='mb-4 flex-1 text-left text-xs text-ink/60 line-clamp-2'>{stripMarkdown(note.content) || 'No additional content yet.'}</button>
                ...
                <div className='flex items-center justify-between'>
                    ...
                    {activeMenu === note.id && (
                        <div className='absolute right-0 top-7 z-20 w-40 rounded-lg border border-black/10 bg-white shadow-lg'>
                            <button type='button' className="block w-full px-4 py-2 text-left text-sm hover:bg-sand/40" onClick={()=> {alert('archive clicked'); setActiveMenu(null);}}>Archive</button>
                            <button type='button'className='block w-full px-4 py-2 text-left text-sm hover:bg-sand/40' onClick={()=>{alert('delete clicked'); setActiveMenu(null)}}>Delete</button>
                        </div>
                    )}
                </div>
                </div>
